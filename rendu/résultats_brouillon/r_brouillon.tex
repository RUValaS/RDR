...
A = A + poids*randn(size(A));
	...
	$$
		H_{j,q}^{err} =  \exp(-j\frac{2\pi}{\lambda}(\Delta z_j\dotproduct\mathbf{I}_q + \epsilon)) =  \exp(-j\frac{2\pi}{\lambda}\Delta z_j\dotproduct\mathbf{I}_q)\exp(-j\frac{2\pi}{\lambda}\epsilon)
	$$
...
%		\caption{Evolution du PSNR pour $<\epsilon^2>=5$mm}
%		\label{fig:h5e-3}
%	\end{figure}
%	
%	\begin{figure}[H]
...
%		\caption{Evolution du PSNR pour $<\epsilon^2>=1$mm}
%		\label{fig:he-3}
%	\end{figure}
%	
%	\begin{figure}[H]
...
%		\caption{Evolution du PSNR pour $<\epsilon^2>=1$cm}
%		\label{fig:he-2}
%	\end{figure}
	...
	Les mesures obtenues ont \emph{généralement} cette forme : on obtient parfois des évolutions sans le pic au début, mais la mesure va toujours converger vers une valeur identique tant que $<\epsilon^2>$ reste constant, en oscillant autour de cette valeur. On peut donc tracer l'évolution de l'erreur de reconstruction en fonction de $<\epsilon^2>$ :
